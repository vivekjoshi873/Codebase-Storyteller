...
const EXAMPLES = ["pallets/click", "tiangolo/fastapi", "pydantic/pydantic", "encode/httpx"];
...
export default function App() {
  ...
  const pollRepoStatus = async (id) => {
    while (true) {
      ...
      if (data.status === "done") {
        ...
        setStatus(`Done - ${data.graph?.nodes?.length ?? 0} files, ${data.graph?.edges?.length ?? 0} imports.`);
        return;
      }
  ...
  const runAnalyse = async (nextUrl) => {
